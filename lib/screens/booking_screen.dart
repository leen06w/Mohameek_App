...
class _BookingScreenState extends State<BookingScreen> {
  ...
  Future<void> _submitRequest() async {
    ...
    try {
      // 1. جلب بيانات الطالب الحالي من الفايربيس
      final currentUser = await AuthService().getCurrentUser();

      if (currentUser == null) {
        _showMessage('يرجى تسجيل الدخول أولاً', success: false);
        return;
      }

      // 2. تجهيز بيانات القضية للجدول الموحد (Cases)
      final caseData = {
        ...
        'consultationType': selectedTypeLabel,
        ...
        'price': lawyer.price,
        'status': 'قيد الانتظار',
        'createdAt': FieldValue.serverTimestamp(), // توقيت الفايربيس الرسمي
      };

      // 3. الحفظ المباشر في مجموعة Cases
      await FirebaseFirestore.instance.collection('Cases').add(caseData);

      if (!mounted) return;
      setState(() => submitting = false);

      // 4. إظهار رسالة النجاح والتحويل التلقائي
      await _showCenteredSuccessAndGoToRequests();
    } catch (e) {
      setState(() => submitting = false);
      _showMessage('حدث خطأ أثناء إرسال الطلب: $e', success: false);
    }
  }
